...
window.UI = (function () {
  ...
  function opciones(nombre, lista, valor) {
    return '<div class="g-opciones" data-opciones="' + nombre + '">' + lista.map(function (o) {
      var icono = o.icono ? (Iconos.resolver(o.icono) || Iconos.resolver(o.id) || o.icono) : '';
      return '<button type="button" class="g-opcion' + (o.id === valor ? ' activa' : '') + '" ' +
        'data-opcion="' + nombre + '" data-valor="' + R.escapar(o.id) + '">' +
        (icono ? '<span class="g-opcion-icono">' + icono + '</span>' : '') +
  ...
  /* Medidor circular en SVG. `sufijo` por defecto es «%», que va junto al
     número; otro sufijo (p. ej. «/ 100») se muestra debajo. */
  function anillo(pct, color, tamano, sufijo) {
    /* Sin color de estado, el avance lleva el degradado del orbe (definido en index.html) */
    var c = color || 'url(#pmbok-orbe)';
    var v = Math.max(0, Math.min(100, Math.round(Number(pct) || 0)));
    var enLinea = sufijo === undefined || sufijo === '%';
    return '<div class="pa-dial ' + (tamano || '') + '" style="--v:' + v + ';--c:' + c + '" ' +
      'role="img" aria-label="' + v + (enLinea ? ' %' : ' ' + R.escapar(sufijo)) + '">' +
      '<svg viewBox="0 0 40 40" aria-hidden="true" focusable="false">' +
        '<circle class="pa-dial-pista" cx="20" cy="20" r="17"></circle>' +
        (v > 0 ? '<circle class="pa-dial-valor" cx="20" cy="20" r="17" pathLength="100"></circle>' : '') +
      '</svg>' +
      '<div class="pa-dial-centro" aria-hidden="true"><b data-contar="' + v + '">' + v +
        (enLinea ? '<small>%</small>' : '') + '</b>' +
        (enLinea ? '' : '<span>' + R.escapar(sufijo) + '</span>') + '</div></div>';
  }
  ...
  function barra(pct, color) {
    ...
    return '<div class="pa-barra" role="presentation"><i style="transform:scaleX(' + f.toFixed(3) + ');background:' +
      (color || 'var(--grad-avance)') + '"></i></div>';
  }

  /* Veredicto de calidad: correcto en cian, vigilar en ámbar, actuar en rojo */
  function colorPorcentaje(p) {
    return p >= 75 ? 'var(--ok)' : p >= 40 ? 'var(--ambar)' : 'var(--rojo)';
  }

  function cifra(valor, etiquetaTexto, color, icono) {
    var entero = /^\d+$/.test(String(valor));
    return '<div class="cifra">' +
      '<div class="cifra-cab">' +
        (icono ? '<span class="cifra-icono">' + Iconos.svg(icono) + '</span>' : '') +
        '<span>' + R.escapar(etiquetaTexto) + '</span>' +
      '</div>' +
      '<b' + (color ? ' style="color:' + color + '"' : '') + (entero ? ' data-contar="' + valor + '"' : '') + '>' +
        valor + '</b></div>';
  }
  ...
  function vacio(glifo, titulo, texto, accion) {
    var icono = window.Iconos ? Iconos.resolver(glifo) : '';
    return '<div class="vacio"><div class="glifo" aria-hidden="true">' + (icono || glifo) + '</div>' +
      '<h3 class="vacio-titulo">' + R.escapar(titulo) + '</h3>' +
