...
window.ObraAcciones = (function () {
  ...
  function manejar(e) {
    ...
    var accion = el.getAttribute('data-o');
    var id = el.getAttribute('data-id');
    var val = el.getAttribute('data-valor');
    e.preventDefault();
    ...
    switch (accion) {
      ...
      case 'abrir-nuevo-doc': alternar('g-nuevo-doc', true); break;
      case 'cerrar-nuevo-doc': alternar('g-nuevo-doc', false); break;

      case 'crear-doc': {
        var art = valor('nd-artefacto');
        if (!art) return;
        var rd = Gestor.generarDocumento(proyectoId, art, null);
        if (rd.error) { Dialogo.avisar(rd.error, 'error'); return; }
        location.hash = '#/proyectos/' + proyectoId + '/documento/' + rd.documento.id;
        recargar();
        break;
      }
      ...
      case 'doc-borrar': {
        ...
        Dialogo.confirmar({
          titulo: 'Eliminar «' + (docBorrar ? docBorrar.nombre : 'documento') + '»',
          ...
          Gestor.borrar('documentos', docBorrar.id);
      ...
      case 'doc-exportar':
        exportarDocumento(valor('g-doc-id'));
        break;
      ...
      case 'abrir-archivo': Archivos.abrir(id); break;
      case 'descargar-archivo': Archivos.descargar(id); break;
      case 'borrar-archivo': {
      ...
      case 'nuevo-sprint': {
        ...
        var sprintsPrevios = Gestor.lista('sprints', { proyectoId: proyectoId });
        var hayCero = sprintsPrevios.some(function (x) { return /^Sprint 0(?:\s|$)/.test(x.nombre); });
        var siguienteN = sprintsPrevios.length + (hayCero ? 0 : 1);
        Dialogo.pedir({
          titulo: 'Nuevo sprint',
      ...
      case 'cerrar-sprint': {
        var aCerrar = Gestor.uno('sprints', id);
        Dialogo.confirmar({
          titulo: 'Cerrar «' + (aCerrar ? aCerrar.nombre : 'sprint') + '»',
      ...
      case 'borrar-tarea':
        borrarConDeshacer('tareas', id, 'Elemento del backlog');
        break;
      ...
      case 'borrar-riesgo':
        borrarConDeshacer('riesgos', id, 'Riesgo');
        break;

      case 'crear-interesado': {
        var ni = valor('ni-nombre');
        if (!ni.trim()) return;
      ...
      case 'borrar-interesado':
        borrarConDeshacer('interesados', id, 'Interesado');
        break;

      case 'crear-cambio': {
        var nc = valor('nc-titulo');
        if (!nc.trim()) return;
        Gestor.crear('cambios', {
          proyectoId: proyectoId, titulo: nc.trim(), descripcion: valor('nc-descripcion'),
      ...
      case 'crear-leccion': {
        var nl = valor('nl-situacion');
        if (!nl.trim()) return;
        Gestor.crear('lecciones', {
          proyectoId: proyectoId, situacion: nl.trim(), causa: valor('nl-causa'),
      ...
      case 'borrar-leccion':
        borrarConDeshacer('lecciones', id, 'Lección');
        break;
      ...
      case 'borrar-medicion':
        borrarConDeshacer('mediciones', id, 'Medición');
        break;
      ...
      case 'guardar-proyecto': {
        ...
        var actualP = p();
        var cambios = {
          nombre: valor('cp-nombre') || actualP.nombre,
        ...
        if (metNueva && metNueva !== actualP.metodologia) {
          ...
          var met = Gestor.metodologia(metNueva);
          cambios.fases = met.fases.map(function (f, n) {
      ...
      case 'borrar-proyecto': {
        var prBorrar = p();
        Dialogo.pedir({
          titulo: 'Eliminar «' + prBorrar.nombre + '»',
